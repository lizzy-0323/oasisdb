...
public:
  ...
  mutable std::atomic<long> query_count{0};
  ...
  float getAvgHops() { return (float)metric_hops / query_count; }

  float getAvgDistComputations() {
    return (float)metric_distance_computations / query_count;
  }

  size_t getQueryCount() { return query_count; }
...
//                    if (candidate_id == 0) continue;
  ...
  searchBaseLayerST(
    ...
    while (!candidate_set.empty()) {
      ...
      bool flag_stop_search; // 是否需要停止搜索
      if (bare_bone_search) {
        // 没有删除条件下，采用该条件停止搜索
...
//                    if (candidate_id == 0) continue;
        ...
        if (!(visited_array[candidate_id] == visited_array_tag)) {
          ...
          if (!bare_bone_search && stop_condition) {
            ...
            // 是否考虑加入该点到candidate list，取决于top candidate
            // list的大小，或者当前点的距离是否小于lowerBound
            flag_consider_candidate =
